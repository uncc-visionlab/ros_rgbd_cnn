...
class model(nn.Module):
    def __init__(self, num_classes=37, pretrained=False):
        ...
        self.agent0 = self._make_agent_layer(64, 64, stride=1, padding=0)
        self.agent1 = self._make_agent_layer(64 * 4, 64, stride=1, padding=0)
        self.agent2 = self._make_agent_layer(128 * 4, 128, stride=1, padding=0)
        self.agent3 = self._make_agent_layer(256 * 4, 256, stride=1, padding=0)
        self.agent4 = self._make_agent_layer(512 * 4, 512, stride=1, padding=0)
    ...
    def _make_agent_layer(self, inplanes, planes, stride, padding):
        ...
        for k, v in pretrain_dict.items():
            if k in state_dict:
                if k.startswith('conv1'):  # the first conv_op
                    ...
                    model_dict[k.replace('conv1', 'conv1_p')] = v #torch.stack([v[:, 0], v[:, 1], v[:, 2], v[:, 0]], 1)
