...
def train():
    train_data = RedNet_data_depth.SUNRGBD(transform=transforms.Compose([RedNet_data_depth.scaleNorm(),
                                                                   RedNet_data_depth.RandomScale((1.0, 1.4)),
                                                                   #RedNet_data.RandomHSV((0.9, 1.1),
                                                                   #                      (0.9, 1.1),
                                                                   #                      (25, 25)),
                                                                   RedNet_data_depth.RandomCrop(image_h, image_w),
                                                                   RedNet_data_depth.RandomFlip(),
                                                                   RedNet_data_depth.ToTensor(),
                                                                   RedNet_data_depth.Normalize()]),
                                     phase_train=True,
    ...
    if args.last_ckpt:
        ...
        model = RedNet_model_depth.RedNet(pretrained=True)
    ...
    for epoch in range(int(args.start_epoch), args.epochs):
        ...
        for batch_idx, sample in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            pred_scales = model(depth, args.checkpoint)
            loss = CEL_weighted(pred_scales, target_scales)
            loss.backward()
            optimizer.step()
            local_count += depth.data.shape[0]
            global_step += 1
