...
parser.add_argument('--epochs', default=100, type=int, metavar='N',  # default 500
...
def train():
    train_data = data_rgbd.SUNRGBD(transform=transforms.Compose([data_rgbd.scaleNorm(),
                                                                   data_rgbd.RandomScale((1.0, 1.4)),
                                                                   data_rgbd.RandomHSV((0.9, 1.1),
                                                                                         (0.9, 1.1),
                                                                                         (25, 25)),
                                                                   data_rgbd.RandomCrop(image_h, image_w),
                                                                   data_rgbd.RandomFlip(),
                                                                   data_rgbd.ToTensor(),
                                                                   data_rgbd.Normalize()]),
                                     phase_train=True,
    ...
    if args.last_ckpt:
        model = model_rgbd.model(pretrained=False)
    else:
        model = model_rgbd.model(pretrained=True)
